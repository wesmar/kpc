...
OffsetFinder::OffsetFinder()
{
    ...
    if (!m_kernelModule) {
        ERROR(OBFERR(L"OffsetFinder: Failed to load kernel image (error: %d) - verify administrator privileges").c_str(), GetLastError());
    }
}
...
bool OffsetFinder::FindKernelPsInitialSystemProcessOffset() noexcept
{
    ...
    if (!m_kernelModule) {
        ERROR(OBFERR(L"Cannot find PsInitialSystemProcess - kernel image not loaded").c_str());
        return false;
    }
    ...
    if (!pPsInitialSystemProcess) {
        ERROR(OBFERR(L"PsInitialSystemProcess export not found (error: %d)").c_str(), GetLastError());
        ...
        if (GetProcAddress(m_kernelModule.get(), testExport.c_str())) {
            ERROR(OBFERR(L"Other kernel exports accessible - partial export table issue").c_str());
        } else {
            ERROR(OBFERR(L"No kernel exports accessible - incompatible kernel image").c_str());
        }
        return false;
    }
    ...
    if (offset < 0x1000 || offset > 0x2000000) { 
        ERROR(OBFERR(L"PsInitialSystemProcess offset 0x%x outside reasonable range").c_str(), offset);
        return false;
    }
    
    m_offsetMap[Offset::KernelPsInitialSystemProcess] = offset;
    SUCCESS(OBFSUCCESS(L"Found PsInitialSystemProcess offset: 0x%x").c_str(), offset);
    return true;
}
...
bool OffsetFinder::FindProcessUniqueProcessIdOffset() noexcept
{
    ...
    if (!pPsGetProcessId) {
        ERROR(OBFERR(L"PsGetProcessId export not found (error: %d)").c_str(), GetLastError());
        return false;
    }
    ...
    if (!offset) {
        ERROR(OBFERR(L"Failed to extract UniqueProcessId offset from PsGetProcessId function").c_str());
        return false;
    }

    // Enhanced sanity check for modern EPROCESS structure (expanded range)
    if (offset.value() > 0x1500) { 
        ERROR(OBFERR(L"UniqueProcessId offset 0x%x appears too large for EPROCESS").c_str(), offset.value());
        return false;
    }

    m_offsetMap[Offset::ProcessUniqueProcessId] = offset.value();
    SUCCESS(OBFSUCCESS(L"Found UniqueProcessId offset: 0x%x").c_str(), offset.value());
    return true;
}
...
bool OffsetFinder::FindProcessProtectionOffset() noexcept
{
    ...
    if (!pPsIsProtectedProcess || !pPsIsProtectedProcessLight) {
        ERROR(OBFERR(L"Protection function exports not found in kernel image").c_str());
        return false;
    }
    ...
    if (!offsetA || !offsetB) {
        ERROR(OBFERR(L"Failed to extract offsets from protection validation functions").c_str());
        return false;
    }
    ...
    if (offsetA.value() != offsetB.value() || offsetA.value() > 0x1500) { 
        ERROR(OBFERR(L"Protection offset validation failed: A=0x%x, B=0x%x").c_str(), offsetA.value(), offsetB.value());
        return false;
    }

    m_offsetMap[Offset::ProcessProtection] = offsetA.value();
    SUCCESS(OBFSUCCESS(L"Found ProcessProtection offset: 0x%x").c_str(), offsetA.value());
    return true;
}
