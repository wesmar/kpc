...
bool KpcDrv::Initialize() noexcept {
    ...
    if (m_deviceName.empty()) {
        ...
        m_deviceName = devicePrefix + GetServiceName();  // Must match service name (kmpdc)
    }
...
std::optional<BYTE> KpcDrv::Read8(ULONG_PTR address) noexcept {
    auto value = Read32(address);
    if (!value.has_value()) return std::nullopt;
    return static_cast<BYTE>(value.value() & 0xff);
}

std::optional<WORD> KpcDrv::Read16(ULONG_PTR address) noexcept {
    auto value = Read32(address);
    if (!value.has_value()) return std::nullopt;
    return static_cast<WORD>(value.value() & 0xffff);
}

std::optional<DWORD> KpcDrv::Read32(ULONG_PTR address) noexcept {
    return Read(address, sizeof(DWORD));
}

std::optional<DWORD64> KpcDrv::Read64(ULONG_PTR address) noexcept {
...
std::optional<ULONG_PTR> KpcDrv::ReadPtr(ULONG_PTR address) noexcept {
#ifdef _WIN64
...
bool KpcDrv::Write8(ULONG_PTR address, BYTE value) noexcept {
    return Write(address, sizeof(value), value);
}

bool KpcDrv::Write16(ULONG_PTR address, WORD value) noexcept {
    return Write(address, sizeof(value), value);
}

bool KpcDrv::Write32(ULONG_PTR address, DWORD value) noexcept {
    return Write(address, sizeof(value), value);
}

bool KpcDrv::Write64(ULONG_PTR address, DWORD64 value) noexcept {
...
std::optional<DWORD> KpcDrv::Read(ULONG_PTR address, DWORD valueSize) noexcept {
...
bool KpcDrv::Write(ULONG_PTR address, DWORD valueSize, DWORD value) noexcept {
