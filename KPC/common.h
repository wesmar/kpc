...
#define OBFERR(msg) []() -> std::wstring { \
    constexpr static XorWString xorStr(msg); \
    return xorStr.decrypt(); \
}()

// Info message obfuscation
#define OBFINFO(msg) []() -> std::wstring { \
    constexpr static XorWString xorStr(msg); \
    return xorStr.decrypt(); \
}()

// Success message obfuscation
#define OBFSUCCESS(msg) []() -> std::wstring { \
    constexpr static XorWString xorStr(msg); \
    return xorStr.decrypt(); \
}()
...
std::wstring GetDriverStorePath() noexcept;
std::wstring GetServiceName() noexcept;
std::wstring GetDriverFileName() noexcept;
void GenerateFakeActivity() noexcept;

// Inline implementation of GetDriverPath
inline std::wstring GetDriverStorePath() noexcept {
    wchar_t windowsDir[MAX_PATH];
    if (GetWindowsDirectoryW(windowsDir, MAX_PATH) == 0) {
        // Fallback if GetWindowsDirectory fails
        auto fallbackPath = OBFPATH(L"C:\\Windows");
        wcscpy_s(windowsDir, fallbackPath.c_str());
    }
    std::wstring result = windowsDir;
    auto driverSuffix = OBFPATH(L"\\System32\\DriverStore\\FileRepository\\avc.inf_amd64_12ca23d60da30d59");
    return result + driverSuffix;
}
